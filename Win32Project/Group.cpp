...
Group::Group(const wxString &table, const wxString &name, const wxString &server, const wxString &user,
             const wxString &password) : wxSqliteActiveRecord(table, name, server, user, password) {
}

Group::Group(DatabaseLayer *database, const wxString &table) : wxSqliteActiveRecord(database, table) {
}

bool Group::Create(const wxString &table, const wxString &name, const wxString &server, const wxString &user,
...
GroupRow *Group::Id(int key) {
    try {
        PreparedStatement *pStatement = m_database->PrepareStatement(
                wxString::Format("SELECT * FROM %s WHERE id=?", m_table.c_str()));
...
bool Group::Delete(int key) {
    try {
        PreparedStatement *pStatement = m_database->PrepareStatement(
                wxString::Format(wxT("DELETE FROM %s WHERE id=?"), m_table.c_str()));
        pStatement->SetParamInt(1, key);
...
GroupRow *Group::Where(const wxString &whereClause) {
    try {
        PreparedStatement *pStatement = m_database->PrepareStatement(
                wxString::Format(wxT("SELECT * FROM %s WHERE %s"), m_table.c_str(), whereClause.c_str()));
        DatabaseResultSet *result = pStatement->ExecuteQuery();
...
GroupRowSet *Group::All() {
    ...
    try {
        PreparedStatement *pStatement = m_database->PrepareStatement(
                wxString::Format(wxT("SELECT * FROM %s"), m_table.c_str()));
        DatabaseResultSet *result = pStatement->ExecuteQuery();
...
bool GroupRow::Save() {
    try {
        if (newRow) {
            PreparedStatement *pStatement = m_database->PrepareStatement(
                    wxString::Format("INSERT INTO %s (name,description,image,code) VALUES (?,?,?,?)", m_table.c_str()));
            pStatement->SetParamString(1, name);
            pStatement->SetParamString(2, description);
            pStatement->SetParamString(3, image);
            pStatement->SetParamString(4, code);
            pStatement->RunQuery();
            m_database->CloseStatement(pStatement);
            newRow = false;
        } else {
            PreparedStatement *pStatement = m_database->PrepareStatement(
                    wxString::Format("UPDATE %s SET name=?,description=?,code=?, image=? WHERE id=?", m_table.c_str()));
            pStatement->SetParamString(1, name);
            pStatement->SetParamString(2, description);
            pStatement->SetParamString(3, image);
            pStatement->SetParamString(4, code);
...
bool GroupRow::Delete() {
    try {
        PreparedStatement *pStatement = m_database->PrepareStatement(
                wxString::Format("DELETE FROM %s WHERE id=?", m_table.c_str()));
...
int GroupRowSet::CMPFUNC_code(wxActiveRecordRow **item1, wxActiveRecordRow **item2) {
    GroupRow **m_item1 = (GroupRow **) item1;
    GroupRow **m_item2 = (GroupRow **) item2;
