...
bool GroupRow::Save(){
	try{
		if(newRow){
			PreparedStatement* pStatement=m_database->PrepareStatement(wxString::Format("INSERT INTO %s (name,description,image,code) VALUES (?,?,?,?)",m_table.c_str()));
			...
			PreparedStatement* pStatement=m_database->PrepareStatement(wxString::Format("UPDATE %s SET name=?,description=?,code=?, image=? WHERE id=?",m_table.c_str()));
			pStatement->SetParamString(1,name);
			pStatement->SetParamString(2,description);
			pStatement->SetParamString(3,image);
            pStatement->SetParamString(4,code);
