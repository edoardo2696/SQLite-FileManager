...
bool Gcode::Create(const wxString &table, const wxString &name, const wxString &server, const wxString &user,
...
GcodeRow *Gcode::Id(int key) {
    try {
        PreparedStatement *pStatement = m_database->PrepareStatement(
                wxString::Format(wxT("SELECT * FROM %s WHERE id=?"), m_table.c_str()));
...
bool GcodeRow::Save() {
    try {
        if (newRow) {
            PreparedStatement *pStatement = m_database->PrepareStatement(
                    wxString::Format(wxT("INSERT INTO %s (elementid, code) VALUES (?,?)"), m_table.c_str()));
            ...
            PreparedStatement *pStatement = m_database->PrepareStatement(
                    wxString::Format(wxT("UPDATE %s SET code=? ,elementid=? WHERE id=?"), m_table.c_str()));
            pStatement->SetParamInt(1, elementid);
            pStatement->SetParamString(2, code);
